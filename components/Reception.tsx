...
export const Reception: React.FC<ReceptionProps> = ({ lang, currentUser }) => {
  ...
  const [isFilterOpen, setIsFilterOpen] = useState(true); // State for collapse/expand
        ...
        <div className="p-4 space-y-3 border-b border-slate-100 dark:border-slate-700">
            ...
            <div className="border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/50">
                ...
                <button 
                    ...
                    className="w-full flex justify-between items-center px-3 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 transition-colors"
                >
                    <div className="flex items-center gap-2">
                        <Filter size={16} /> <span>{t.filterBy}</span>
                    </div>
                    {/* Chevron icon: rotates 90deg when open */}
                    <ArrowRight size={16} className={`text-slate-400 transition-transform duration-300 ${isFilterOpen ? 'rotate-90' : 'rotate-0'}`} />
                </button>
                
                {/* Collapsible Content */}
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isFilterOpen ? 'max-h-60 opacity-100 border-t border-slate-200 dark:border-slate-700' : 'max-h-0 opacity-0'}`}>
        ...
        {!currentStudent ? <div className="flex-1 flex flex-col items-center justify-center text-slate-400"><div className="w-32 h-32 rounded-full bg-white/80 dark:bg-slate-800/80 flex items-center justify-center mb-4 shadow-sm"><UserCheck size={64} className="opacity-20" /></div><h2 className="text-xl font-bold text-slate-500 dark:text-slate-400">{t.readyToProcess}</h2><p className="text-sm opacity-70">{t.selectStudentMsg}</p></div> : (
            <div className="flex-1 flex flex-col min-h-0">
                <div className="p-6 pb-2 shrink-0">
                    <Card className="bg-white dark:bg-slate-800 p-6 shadow-lg border-2 border-slate-100 dark:border-slate-700">
                        <h2 className="text-2xl font-extrabold text-slate-800 dark:text-white">{lang === 'en' ? currentStudent.name_en : currentStudent.name_ar}</h2>
                        <p className="text-slate-500 dark:text-slate-400 font-mono text-sm">Student ID: {currentStudent.studentNumber}</p>
                        <div className="mt-4 flex flex-wrap items-center gap-3">
                            <Badge color="gray" className="text-xs font-semibold">{currentStudent.grade} - {currentStudent.section}</Badge>
                            <div className="flex items-center gap-1 text-sm text-slate-600 dark:text-slate-300">
                                {currentStudent.transportMode === 'Bus' ? <Bus size={14} className="text-blue-500 dark:text-blue-400" /> : <MapPin size={14} className="text-blue-500 dark:text-blue-400" />}
                ...
                <div className="flex-1 overflow-y-auto px-6 py-2">{mode === 'LateArrival' ? <div className="h-full flex flex-col justify-center items-center text-center space-y-4 bg-white/50 dark:bg-slate-800/50 rounded-xl border border-white dark:border-slate-700 shadow-sm p-8"><div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center animate-pulse"><Clock size={32} /></div><div><h3 className="text-lg font-bold text-slate-700 dark:text-slate-200">{t.confirmLate}</h3><p className="text-slate-500 dark:text-slate-400 text-sm mt-1 max-w-xs">{t.confirmLateMsg}</p></div></div> : <div className="space-y-4 pb-4">
                    <Card className="border-0 shadow-sm bg-white/80 dark:bg-slate-800/80">
                        <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-3 flex items-center gap-2"><AlertTriangle size={16} className="text-orange-500" /> {t.reason}</h4>
                        <div className={reasonSelect === 'Other' ? "grid grid-cols-1 md:grid-cols-2 gap-4" : "w-full"}>
                            <Select value={reasonSelect} onChange={(e) => setReasonSelect(e.target.value)} className="mb-0">
                                <option value="">{t.selectReason}</option>
                                {EARLY_LEAVE_REASONS.map((r: string) => <option key={r} value={r}>{r}</option>)}
                            </Select>
                            {reasonSelect === 'Other' && <Input placeholder={t.pleaseSpecify} value={reasonText} onChange={(e) => setReasonText(e.target.value)} autoFocus />}
                        </div>
                    </Card>
                    <Card className="border-0 shadow-sm bg-white/80 dark:bg-slate-800/80">
                        <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-3 flex items-center gap-2"><UserCheck size={16} className="text-slate-500 dark:text-slate-400" /> {t.guardianPickup}</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="text-xs font-bold text-slate-500 dark:text-slate-400 block mb-1">{t.pickupBy}</label><Select value={pickedBy} onChange={(e) => setPickedBy(e.target.value)}><option value="">{t.selectRelation}</option>{PICKUP_RELATIONS.map((r: string) => <option key={r} value={r}>{r}</option>)}</Select></div>
                            <div><label className="text-xs font-bold text-slate-500 dark:text-slate-400 block mb-1">{t.pickupId}</label><Input placeholder="Emirates ID / National ID" value={pickerId} onChange={(e) => setPickerId(e.target.value)} /></div>
                        </div>
                    </Card>
                    <div className="flex items-start gap-3 p-4 rounded-lg bg-orange-100/50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 text-orange-800 dark:text-orange-300 text-xs"><Info size={16} className="shrink-0 mt-0.5" /><p>{t.autoAttendance}</p></div>
                </div>}</div>
                <div className="shrink-0 p-4 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center z-20">
                    <Button variant="ghost" onClick={() => setSelectedStudentId("")} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">{t.cancel}</Button>
                    <Button 
                        onClick={handleLog} 
                        disabled={(mode === 'EarlyLeave' && (!reasonSelect || !pickedBy || !pickerId)) || isLoading} 
                        className={`px-6 py-2 text-base shadow-xl transition-transform active:scale-95 ${mode === 'LateArrival' ? 'bg-blue-600 hover:bg-blue-700 shadow-blue-200 dark:shadow-none' : 'bg-orange-600 hover:bg-orange-700 shadow-orange-200 dark:shadow-none'}`}
                    >
