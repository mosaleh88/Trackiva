      ...
      return (
          <Card className={`border-2 ${emergencyMode ? 'border-red-500 bg-red-50 dark:bg-red-900/20' : 'border-slate-200 dark:border-slate-700'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">{t.symptom} *</label><Select value={formData.symptom} onChange={(e) => setFormData({...formData, symptom: e.target.value})}><option value="">Select...</option>{CLINIC_SYMPTOMS.map(s => <option key={s} value={s}>{s}</option>)}</Select></div>
