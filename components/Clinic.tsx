...
export const Clinic: React.FC<ClinicProps> = ({ lang, currentUser }) => {
  ...
  const renderVisitForm = () => {
      ...
      return (
          <Card className={`border-2 ${emergencyMode ? 'border-red-500 bg-red-50' : 'border-slate-200'}`}>
        ...
        <Card>
            <div className="relative max-w-xl mx-auto z-20">
                ...
                {reportSearch && !reportStudent && (
                    <div className="absolute top-full left-0 right-0 bg-white border border-slate-200 rounded-b-xl shadow-lg mt-1">
                        {reportSearchResults.map(s => (
                            <button key={s.id} onClick={() => { setReportStudent(s); setReportSearch(lang === 'en' ? s.name_en : s.name_ar); }} className="w-full text-left text-start px-4 py-3 hover:bg-slate-50 border-b border-slate-50 last:border-0">
                                <p className="font-bold text-slate-700">{lang === 'en' ? s.name_en : s.name_ar}</p><p className="text-xs text-slate-500">{s.studentNumber} - {s.grade}-{s.section}</p>
                            </button>
                        ))}
             ...
             <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6">
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                     <Card className="md:col-span-2 flex items-center gap-6">
                         <div className="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center text-slate-400"><UserIcon size={40} /></div>
                         <div><h2 className="text-2xl font-bold">{lang === 'en' ? reportStudent.name_en : reportStudent.name_ar}</h2><div className="flex gap-2 mt-1"><Badge color="gray">#{reportStudent.studentNumber}</Badge><Badge color="blue">{reportStudent.grade}-{reportStudent.section}</Badge><Badge color="gray">{reportStudent.gender}</Badge></div></div>
                     </Card>
                     <Card className="flex flex-col justify-center items-center text-center bg-blue-50 border-blue-100"><p className="text-slate-500 font-bold uppercase text-xs tracking-wider">{t.totalVisits}</p><h3 className="text-5xl font-bold text-blue-600 mt-2">{reportVisits.length}</h3></Card>
                 </div>
                 <Card>
                     <h3 className="font-bold text-lg mb-4">{t.medicalHistory}</h3>
                     {reportVisits.length === 0 ? <div className="text-center py-8 text-slate-400">{t.noVisitsFound}</div> : (
                         <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="border-b border-slate-100 text-slate-500 text-sm bg-slate-50"><th className="p-3 rounded-tl-lg text-start">{t.date}</th><th className="p-3 text-start">{t.symptom}</th><th className="p-3 text-start">{t.diagnosis}</th><th className="p-3 text-start">{t.treatment}</th><th className="p-3 text-start">{t.severity}</th><th className="p-3 rounded-tr-lg text-start">{t.outcome}</th></tr></thead><tbody className="divide-y divide-slate-50">{reportVisits.map(v => (<tr key={v.id} className="hover:bg-slate-50"><td className="p-3 text-sm text-start"><div className="font-bold">{new Date(v.timestamp).toLocaleDateString()}</div><div className="text-xs text-slate-400">{new Date(v.timestamp).toLocaleTimeString()}</div></td><td className="p-3 font-medium text-start">{v.symptom}</td><td className="p-3 text-sm text-slate-600 text-start">{v.diagnosis || '-'}</td><td className="p-3 text-sm text-slate-600 text-start">{v.treatment || '-'}</td><td className="p-3 text-start"><Badge color={v.severity === 'Emergency' ? 'red' : v.severity === 'High' ? 'orange' : 'blue'}>{v.severity}</Badge></td><td className="p-3 text-sm text-start">{v.outcome}</td></tr>))}</tbody></table></div>
                     )}
                 </Card>
             </div>
          ...
          {showEmergencyModal && (
              <div className="fixed inset-0 bg-red-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                  <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in duration-200">
                      ...
                      <div className="p-6 space-y-4"><p className="text-slate-600 text-center font-medium">{t.emergencyConfirm}</p><p className="text-slate-500 text-sm text-center bg-slate-50 p-3 rounded-lg border border-slate-100">{t.emergencyConfirmDesc}</p><div className="flex gap-3 mt-4"><button onClick={() => setShowEmergencyModal(false)} className="flex-1 py-3 rounded-xl border border-slate-200 font-bold text-slate-600 hover:bg-slate-50 transition-colors">{t.cancel}</button><button onClick={handleConfirmEmergency} className="flex-1 py-3 rounded-xl bg-red-600 font-bold text-white hover:bg-red-700 shadow-lg shadow-red-200 transition-colors">{t.confirmAction}</button></div></div>
                  </div>
              </div>
          )}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Card className="flex items-center gap-4"><div className="p-3 bg-blue-50 text-blue-600 rounded-full"><Stethoscope size={24} /></div><div><p className="text-sm text-slate-500">{t.todayVisits}</p><h3 className="text-2xl font-bold">{todayVisits.length}</h3></div></Card>
              <Card className="flex items-center gap-4"><div className="p-3 bg-orange-50 text-orange-600 rounded-full"><Activity size={24} /></div><div><p className="text-sm text-slate-500">{t.incomingPatients}</p><h3 className="text-2xl font-bold">{activePasses.length}</h3></div></Card>
              <Card className="flex items-center gap-4"><div className="p-3 bg-red-50 text-red-600 rounded-full"><Siren size={24} /></div><div><p className="text-sm text-slate-500">{t.sentHome}</p><h3 className="text-2xl font-bold">{todayVisits.filter(v => v.outcome === 'SentHome').length}</h3></div></Card>
              <button onClick={() => setShowEmergencyModal(true)} className="flex flex-col items-center justify-center bg-red-600 text-white rounded-xl hover:bg-red-700 transition-colors shadow-lg shadow-red-200 group"><AlertTriangle size={24} className="mb-1 group-hover:scale-110 transition-transform" /><span className="font-bold uppercase text-xs tracking-wider">{t.emergencyAlert}</span></button>
          </div>
          {epidemicAlert.length > 0 && <div className="bg-red-50 border border-red-200 p-4 rounded-xl flex items-start gap-3 animate-in slide-in-from-top"><AlertTriangle className="text-red-600 shrink-0" /><div><h4 className="font-bold text-red-800">{t.epidemicWarning}</h4><p className="text-sm text-red-700 mt-1">{t.epidemicMsg} {epidemicAlert.map(([s, c]) => `${s} (${c})`).join(', ')}</p></div></div>}
          ...
          {activeTab === 'queue' && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="space-y-4">
                      <div className="flex items-center justify-between"><h3 className="font-bold text-slate-700">{t.incomingPatients}</h3><Badge color="blue">{activePasses.length}</Badge></div>
                      {activePasses.length === 0 ? <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-xl text-slate-400"><Coffee size={32} className="mx-auto mb-2 opacity-20" /><p>{t.noIncoming}</p></div> : incomingStudents.map(({ pass, student }) => (<div key={pass.id} className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm flex justify-between items-center"><div><h4 className="font-bold text-slate-800">{lang === 'en' ? student?.name_en : student?.name_ar}</h4><p className="text-xs text-slate-500 font-mono">{new Date(pass.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p></div><Button className="py-1 px-3 text-sm" onClick={() => handleAdmitFromQueue(student!.id, pass.id)}>{t.admitPatient} <ArrowRight size={16} /></Button></div>))}
                      <div className="pt-4 border-t border-slate-100 relative">
                           <p className="text-xs font-bold text-slate-500 mb-2">{t.admitWalkIn}</p>
                           ...
                           {(searchStudent || (walkInGender && walkInGrade && walkInSection)) && <div className="mt-2 bg-white border border-slate-200 rounded-lg max-h-40 overflow-y-auto shadow-lg absolute z-10 w-full max-w-[calc(100%-2rem)] lg:max-w-[22rem] left-0 right-0">{filteredWalkInStudents.length === 0 ? <div className="p-3 text-sm text-slate-400 text-center">No students found</div> : filteredWalkInStudents.map(s => (<button key={s.id} onClick={() => { setSelectedStudentId(s.id); setSearchStudent(""); setWalkInGender(""); setWalkInGrade(""); setWalkInSection(""); }} className="w-full text-left px-4 py-3 hover:bg-blue-50 text-sm border-b border-slate-50 last:border-0 flex justify-between items-center"><span className="font-medium text-slate-700">{s.name_en}</span><span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded">{s.studentNumber}</span></button>))}</div>}
                           {!selectedStudentId && !searchStudent && !walkInGender && <div className="mt-2 bg-blue-50 border border-blue-100 rounded-lg p-3 flex items-start gap-2"><Plus className="text-blue-500 shrink-0 w-4 h-4 mt-0.5" /><p className="text-xs text-blue-700">Search for a student or select their class to admit them immediately without an E-Pass.</p></div>}
                      </div>
                  </div>
                  <div className="lg:col-span-2">{selectedStudentId ? renderVisitForm() : <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-xl text-slate-400 bg-slate-50 min-h-[300px]"><FileText size={48} className="opacity-20 mb-4" /><p className="text-sm font-medium">Select a student from the queue or search to log a visit</p></div>}</div>
              </div>
          )}
          {activeTab === 'history' && <Card><h3 className="font-bold text-lg mb-4">{t.medicalHistory}</h3><div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="bg-slate-50 text-slate-500 text-sm border-b"><th className="p-3 text-start">{t.date}</th><th className="p-3 text-start">{t.studentName}</th><th className="p-3 text-start">{t.admitted}</th><th className="p-3 text-start">{t.discharged}</th><th className="p-3 text-start">{t.symptom}</th><th className="p-3 text-start">{t.severity}</th><th className="p-3 text-start">{t.outcome}</th></tr></thead><tbody>{visits.length === 0 ? <tr><td colSpan={7} className="p-6 text-center text-slate-400 italic">No records found</td></tr> : visits.map(v => { const s = students.find(stu => stu.id === v.studentId); return (<tr key={v.id} className="border-b border-slate-100 hover:bg-slate-50"><td className="p-3 text-sm text-start">{new Date(v.timestamp).toLocaleDateString()}</td><td className="p-3 font-medium text-start">{lang === 'en' ? s?.name_en : s?.name_ar}</td><td className="p-3 text-sm font-mono text-slate-500 text-start">{new Date(v.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</td><td className="p-3 text-sm font-mono text-slate-500 text-start">{v.dischargeTime ? new Date(v.dischargeTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : '-'}</td><td className="p-3 text-start">{v.symptom}</td><td className="p-3 text-start"><Badge color={v.severity === 'Emergency' || v.severity === 'High' ? 'red' : 'blue'}>{v.severity}</Badge></td><td className="p-3 text-sm text-start">{v.outcome}</td></tr>)})}</tbody></table></div></Card>}
          {activeTab === 'report' && renderReportTab()}
          {activeTab === 'analytics' && <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Card className="h-80 flex flex-col"><h3 className="font-bold mb-4">{t.commonComplaints}</h3><div className="flex-1 min-h-0"><ResponsiveContainer width="100%" height="100%" minWidth={0}><BarChart data={symptomData} layout="vertical"><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" /><YAxis dataKey="name" type="category" width={100} fontSize={12} /><Tooltip /><Bar dataKey="value" fill="#3b82f6" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer></div></Card><Card className="h-80 flex flex-col"><h3 className="font-bold mb-4">{t.visitsByGrade}</h3><div className="flex-1 min-h-0"><ResponsiveContainer width="100%" height="100%" minWidth={0}><BarChart data={gradeData}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" fontSize={12} /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="value" fill="#8b5cf6" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div></Card><Card className="h-80 flex flex-col"><h3 className="font-bold mb-4">{t.visitsBySection}</h3><div className="flex-1 min-h-0"><ResponsiveContainer width="100%" height="100%" minWidth={0}><BarChart data={sectionData}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" fontSize={12} /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="value" fill="#f59e0b" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div></Card><Card className="h-80 flex flex-col"><h3 className="font-bold mb-4">{t.visitsByGender}</h3><div className="flex-1 min-h-0"><ResponsiveContainer width="100%" height="100%" minWidth={0}><BarChart data={genderData}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" fontSize={12} /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="value" fill="#ec4899" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div></Card></div>}
      </div>
  );
};