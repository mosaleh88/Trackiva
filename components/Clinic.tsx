  ...
  return (
      <div className="space-y-6 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Card className="flex items-center gap-4 min-w-0"><div className="p-3 bg-blue-50 text-blue-600 rounded-full"><Stethoscope size={24} /></div><div><p className="text-sm text-slate-500">{t.todayVisits}</p><h3 className="text-2xl font-bold">{todayVisits.length}</h3></div></Card>
              <Card className="flex items-center gap-4 min-w-0"><div className="p-3 bg-orange-50 text-orange-600 rounded-full"><Activity size={24} /></div><div><p className="text-sm text-slate-500">{t.incomingPatients}</p><h3 className="text-2xl font-bold">{activePasses.length}</h3></div></Card>
              <Card className="flex items-center gap-4 min-w-0"><div className="p-3 bg-red-50 text-red-600 rounded-full"><Siren size={24} /></div><div><p className="text-sm text-slate-500">{t.sentHome}</p><h3 className="text-2xl font-bold">{todayVisits.filter(v => v.outcome === 'SentHome').length}</h3></div></Card>
              <button onClick={() => setShowEmergencyModal(true)} className="flex flex-col items-center justify-center bg-red-600 text-white rounded-xl hover:bg-red-700 transition-colors shadow-lg shadow-red-200 group min-w-0"><AlertTriangle size={24} className="mb-1 group-hover:scale-110 transition-transform" /><span className="font-bold uppercase text-xs tracking-wider">{t.emergencyAlert}</span></button>
          </div>
