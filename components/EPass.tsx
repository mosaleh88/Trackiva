...
export const EPass: React.FC<EPassProps> = ({ lang, currentUserRole, currentUserId }) => {
  ...
  }, [currentUserId, store.getStudents()]); // Re-run if store students change

  // Filter passes for visible students
                                                ...
                                                {destinations.map((dest: EPassDestination) => {
                                                    ...
                                                    return (
                                                        <button key={dest.id} disabled={isLoading} onClick={() => handleCreatePass(student.id, dest.id)} className={`flex flex-col items-center justify-center p-2 rounded-lg transition-colors gap-1 ${getThemeClasses(dest.colorTheme)} disabled:opacity-50`}>
                                                            <IconComp size={18} />
                                                            <span className="text-[10px] font-bold text-center leading-tight">{lang === 'en' ? dest.label_en : dest.label_ar}</span>
                                                        </button>
                                                    );
                                                })}
                                            </div>
